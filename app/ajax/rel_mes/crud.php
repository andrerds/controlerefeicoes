<?php
	 /* função de cadastro */
	function cadastro($total ,$data, $mes, $ano){
	    $pdo1 = conecta();
	    try {
	        $cadastro = $pdo1->prepare('INSERT INTO total_mes (total, data, mes, ano) 
	        														VALUE(?,?,?,?)');
	        ...
	        $cadastro = $pdo2->prepare('INSERT INTO total_mes (total, data, mes, ano) 
	        														VALUE(?,?,?,?)');
	        $cadastro->bindValue(1, $total, PDO::PARAM_STR);
	       	$cadastro->bindValue(2, $data, PDO::PARAM_STR);
	        $cadastro->bindValue(3, $mes, PDO::PARAM_STR);
	...
	function listar(){
	    $pdo1 = conecta();
	    try {
	   		$listar = $pdo1 -> query("SELECT * FROM total_mes ORDER BY mes DESC");
	      
  			$pdo2 = conecta2();
	  
	        $listar = $pdo2 -> query("SELECT * FROM total_mes ORDER BY mes DESC");
	...
	function pegarid($id){
		...
		//funcçao atualiza
	function atualizar($total, $data, $mes, $ano, $id){
		 $pdo1 = conecta();
	    try {
	        $atualizar = $pdo1 -> prepare("UPDATE total_mes SET total=?, data=?, mes=?, ano=? WHERE id =?");
			$atualizar->bindValue(1, $total, PDO::PARAM_STR);
	        ...
	        $atualizar = $pdo2 -> prepare("UPDATE total_mes SET total=?, data=?, mes=?, ano=? WHERE id =?");
			$atualizar->bindValue(1, $total, PDO::PARAM_STR);
			$atualizar->bindValue(2, $data, PDO::PARAM_STR);
			$atualizar->bindValue(3, $mes, PDO::PARAM_STR);
			$atualizar->bindValue(4, $ano, PDO::PARAM_STR);
