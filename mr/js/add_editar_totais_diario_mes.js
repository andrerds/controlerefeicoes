...
$(document).ready(function() {
	...
	janela.click(function() {
		$.post('http://houseon.ddns.net/app/ajax/add_total_ml/painel.php',{
			acao : 'form_cad'
		}, function(retorno) {
			$('#myModal').modal({
				backdrop : 'static'
	...
	$("#myModal").on("submit", 'form[name="form_cad"]', function() {
		...
		$.ajax({
			url : 'http://houseon.ddns.net/app/ajax/add_total_ml/controller.php',
			type : 'POST',
			...
			success : function(retorno) {
				...
				if (retorno === 'cadastrou') {
					form.fadeOut('slow', function() {
						...
						listar('http://houseon.ddns.net/app/ajax/add_total_ml/painel.php', 'listar',true);
					});
				} else {
	...
	$('table').on("click", '#b_e_tml_d', function() {
	...
	//BTN DELETAR
	$('table').on("click", '#b_d_tml_d', function(){
		var id = $(this).attr('data-id');
		if(confirm('Realmente quer excluir essa informação ?')){
		$.post('http://houseon.ddns.net/app/ajax/add_total_ml/controller.php', {acao: 'excluir', id: id}, function(retorno){
			if(retorno === 'deletou'){
	...
	listar('http://houseon.ddns.net/app/ajax/add_total_ml/painel.php', 'listar'); 
	//___
