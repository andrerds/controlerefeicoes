$(document).ready(function () {
 ...
 /** FormulÃ¡rio de contato */
    $('form[name="form_cad_mes"]').submit(function(event){
        var form_cad_mes_total = $(this);
        event.preventDefault();
        var mes = form_cad_mes_total.find("#mes").val();
	 ...
	 if (mes.length === 0) {
             open_modal('Opsss! Campo Mes em branco!', 'Você está esquecendo de preencher o campo mes.', 'alerta');
        }   else 
        $.ajax({
            url: 'http://houseon.ddns.net/app/ajax/script_cal/calc_update_total_mes.php',
            data: form_cad_mes_total.serialize(),
            type: 'POST',
            beforeSend: function() {
                form_cad_mes_total.find('.btn').text('Atualizando...').attr('disabled', 'disabled');
                form_cad_mes_total.find('.load').fadeIn('fast');
            },
            ...
            success: function(retorno) {
                ...
                if ($.trim(retorno) === 'empty') {
                    ...
                    open_modal('Que Show !', 'O Mês Foi atualizado , clique no botao refresh.', 'sucesso');
                    form_cad_mes_total[0].reset();
                } else {
            ...
            complete: function() {
                form_cad_mes_total.find('.btn').text('atualizado').removeAttr('disabled');
                form_cad_mes_total.find('.load').fadeOut('fast');
            }
        });
