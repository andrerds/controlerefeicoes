...
$(document).ready(function() {
	...
	$('table').on("click", '#btn_edit_rel_todos_reg', function() {
		var id = $(this).attr('data-id');
			$.post('http://houseon.ddns.net/app/ajax/rel_todos_reg/painel.php', {
			acao : 'form_edit',
			id : id
		}, function(retorno) {
			$('#myModal').modal({
	...
	$("#myModal").on("submit", 'form[name="form_edit"]', function() {
		...
		$.ajax({
			url : 'http://houseon.ddns.net/app/ajax/rel_todos_reg/controller.php',
			type : 'POST',
			...
			success: function(retorno) {
				if (retorno === 'atualizou') {
					dados.fadeOut('slow', function() {
						...
						listar('http://houseon.ddns.net/app/ajax/rel_todos_reg/painel.php', 'listar', true);
					});
				} else {
	...
	//BTN DELETAR
	$('table').on("click", '#btn_del_rel_todos_reg', function(){
		var id = $(this).attr('data-id');
		if(confirm('Realmente quer excluir essa informação ?')){
		$.post('http://houseon.ddns.net/app/ajax/rel_todos_reg/controller.php', {acao: 'excluir', id: id}, function(retorno){
			if(retorno === 'deletou'){
	...
	listar('http://houseon.ddns.net/app/ajax/rel_todos_reg/painel.php', 'listar'); 
	//___
