...
$(document).ready(function() {
	...
	//************************************
	 janela.click(function() {
		$.post('http://houseon.ddns.net/app/ajax/rel_mes/painel.php',{
			acao : 'form_cad'
		}, function(retorno) {
			$('#myModal').modal({
				backdrop : 'static'
	...
	$("#myModal").on("submit", 'form[name="form_cad"]', function() {
		...
		$.ajax({
			url : 'http://houseon.ddns.net/app/ajax/rel_mes/controller.php',
			type : 'POST',
			...
			success : function(retorno) {
				...
				if (retorno === 'cadastrou') {
					form.fadeOut('slow', function() {
						...
						listar('http://houseon.ddns.net/app/ajax/rel_mes/painel.php', 'listar',true);
					});
				} else {
	...
	$('table').on("click", '#btn_del_rel_diario', function(){
		...
		$.post('http://houseon.ddns.net/app/ajax/rel_mes/controller.php', {acao: 'excluir', id: id}, function(retorno){
			if(retorno === 'deletou'){
	...
	listar('http://houseon.ddns.net/app/ajax/rel_mes/painel.php', 'listar'); 
	//___
